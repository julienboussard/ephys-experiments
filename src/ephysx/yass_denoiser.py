...
class SingleChanDenoiser(nn.Module):
    ...
    def train(self, fname_save, DenoTD, n_train=50000, n_test=500, EPOCH=2000, BATCH_SIZE=512, LR=0.0001, device=None):
        """
        ...
        wf_col_train = torch.tensor(wf_col_train, dtype=torch.float)
        wf_clean_train = torch.tensor(wf_clean_train, dtype=torch.float)

        wf_col_test, wf_clean_test = DenoTD.make_training_data(n_test)
        wf_col_test = torch.tensor(wf_col_test, dtype=torch.float)
        wf_clean_test = torch.tensor(wf_clean_test, dtype=torch.float)

        train = Data.TensorDataset(wf_col_train, wf_clean_train)
        train_loader = Data.DataLoader(train, batch_size=BATCH_SIZE, shuffle=True)

        test = Data.TensorDataset(wf_col_test, wf_clean_test)
        ...
        for epoch in range(EPOCH):
            for step, (b_x, b_y) in enumerate(train_loader):   # gives batch data, normalize x when iterate train_loader
                ...
                est = self(b_x.to(device))#[0] 
                loss = loss_func(est, b_y.to(device))   # cross entropy loss b_y.cuda()
                ...
                if step % 100 == 0:
                    with torch.no_grad():
                        ...
                        for (t_x, t_y) in test_loader:
                            t_yhat = self(t_x.cuda())
                            test_loss.append(loss_func(t_yhat, t_y.cuda()).numpy(force=True))
                        test_loss = np.mean(test_loss)
                        test_losses.append(test_loss)
                        print('Epoch: ', epoch, '| train loss: %.4f' % loss.numpy(force=True), '| test loss: %.4f' % test_loss)
                        if epoch > 100:
                            if np.mean(test_losses[-10:]) > np.mean(test_losses[-30:]):
                                print("Stopping early")
                                break
            else:
                # if inner loop broke out
                break
...
class Denoising_Training_Data(object):
    ...
    def make_training_data(self, n):
        ...
        # sample scale
        s1 = np.exp(np.random.randn(n)*0.8 + 2, dtype=self.templates.dtype)
        s2 = np.exp(np.random.randn(n)*0.8 + 2, dtype=self.templates.dtype)

        # turn off some
        c1 = np.random.binomial(1, 1-0.05, n).astype(self.templates.dtype)
        c2 = np.random.binomial(1, 1-0.05, n).astype(self.templates.dtype)
        ...
        # make colliding wf
        wf_clean = np.zeros_like(wf1)
        for j in range(n):
            temp = np.roll(wf1[j], shift[j])
            wf_clean[j] = temp

        # make colliding wf
        wf_col = np.zeros_like(wf2)
        for j in range(n):
...
def make_noise(n, spatial_SIG, temporal_SIG):
    ...
    # get noise
    noise = np.random.normal(size=(n, waveform_length, n_neigh)).astype(spatial_SIG.dtype)
