...
class MatchPursuitObjectiveUpsample:
    ...
    def upsample_templates_mp(self, upsample):
        if upsample != 1:
            ...
            self.up_factor = np.clip(
                int(np.max(self.unit_up_factor)), 1, max_upsample
            )
            self.unit_up_factor[
                self.unit_up_factor > max_upsample
            ] = max_upsample
            self.unit_up_factor = np.maximum(1, self.unit_up_factor).astype(
                int
            )
    ...
    def get_sparse_upsampled_templates(
        self, save_npy=True, return_orig_map=False
    ):
    ...
    def high_res_peak(self, times, unit_ids):
        ...
        """
        if (self.up_factor == 1 and self.no_amplitude_scaling) or len(
            times
        ) < 1:
            return (
                np.zeros_like(times),
        ...
        if self.no_amplitude_scaling:
            ...
            shift_idx = np.argmax(high_res_obj[self.zoom_index, :], axis=0)
    ...
    def find_peaks(self):
        ...
        (
            upsampled_template_idx,
            time_shift,
            valid_idx,
            scalings,
        ) = self.high_res_peak(spike_times, spike_ids)
