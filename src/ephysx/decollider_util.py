...
from dartsort.transform.single_channel_denoiser import SingleChannelDenoiser
from dartsort.util import spikeio
from tqdm.auto import tqdm, trange

from .decollider import SingleChannelPredictor
...
def train_decollider(
    ...
    for epoch in xrange(max_n_epochs):
        ...
        # train
        epoch_losses = []
        for i0 in range(0, len(epoch_data.noisy_waveforms) - batch_size, batch_size):
            tic = time.perf_counter()
...
def evaluate_decollider(
    ...
    val_data = load_epoch(
        ...
        channel_jitter_index=channel_jitter_index,
        examples_per_epoch=(
            None if templates is not None else n_unsupervised_val_examples
        ),
        n_oversamples=n_oversamples,
        noise_same_chans=noise_same_chans,
        noise2_alpha=noise2_alpha,
