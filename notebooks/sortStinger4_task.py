...
with h5py.File(next((dsout / "sub").glob("sub*h5")), "r+" if overwrite else "r") as h5:
    ...
    if "z_reg" in h5 and not overwrite:
        ...
        #     batch_size=64,
        # )
        tme, extra = newt.register(
            maxptp,
            z,
            t,
            raster_kw=dict(gaussian_smoothing_sigma_um=1, post_transform=None),
            weights_kw=dict(
            ...
            thomas_kw=dict(eps=1e-4, lambda_t=0.1),
            max_disp_um=350,
            rigid=True,
        )
        p = tme.displacement
...
fig, (aa, ab) = plt.subplots(ncols=2, figsize=(8, 3))
counts, edges, _ = aa.hist(p, bins=32, color="k")
...
aa.set_xlabel("est. displacement")
aa.set_ylabel("frequency")
aa.axvline(p_mode, color=plt.cm.Greens(0.4), label="target", lw=2, ls="--")
aa.legend()
ab.plot(t_start + np.arange(len(p)), p, c="gray", lw=1)
ab.scatter(t_start + np.arange(len(p))[~p_good], p[~p_good], c="k", s=1, label=f"disp too far {(~p_good).sum()}s", zorder=12)
ab.scatter(t_start + np.arange(len(p))[p_good], p[p_good], color=plt.cm.Greens(0.4), s=1, label=f"disp within pitch/2 of target {(p_good).sum()}s", zorder=12)
ab.set_ylabel("est. displacement")
ab.set_xlabel("time (s)")
ab.legend()
...
# %%
sorting = si.BaseSorting.load_from_folder(dsout / "sisorting")
sorting
...
if (dsout / "siwfs").exists():
    we = si.WaveformExtractor.load_from_folder(dsout / "siwfs")
else:
...
# %%
if (dsout / "sisortingme").exists():
    shutil.rmtree(dsout / "sisortingme")
if (dsout / "siwfsme").exists():
    shutil.rmtree(dsout / "siwfsme")
    
sorting_me = si.NumpySorting.from_times_labels(
    times_list=np.load(dsout / "phy" / "spike_times.npy").squeeze(),
    labels_list=np.load(dsout / "phyME" / "spike_clusters.npy"),
    sampling_frequency=fs
).save(folder=dsout / "sisortingme")
weme = si.extract_waveforms(binrec, sorting_me, dsout / "siwfsme")
metrics_df = si.compute_quality_metrics(we, skip_pc_metrics=True)
metrics_df.to_csv(dsout / f"{humanX}_ME_qualitymetrics.csv")
