...
# %%
# plt.imshow(chunk.T, aspect="auto"); plt.colorbar()
# for c in (0.5, 0.6, 0.7, 0.8, 0.9):
#     p, D, C = ibme.register_raster_rigid(chunk.T, mincorr=c, batch_size=128, disp=40, prior_lambda=1)    
#     plt.plot(50 + p)

# %%
# plt.imshow(chunk.T, aspect="auto"); plt.colorbar()
# p, D, C = ibme.register_raster_rigid(chunk.T, mincorr=0.8, batch_size=128, disp=40, prior_lambda=1)    
# plt.plot(50 + p)

# %%
# online_register_rigid(lf2, batch_length=5000, batch_size=128, prior_lambda=1)
...
# %%
p, D, C = ibme.register_raster_rigid(
    chunk0.T,
    mincorr=0.8,
    prior_lambda=1,
    batch_size=64,
...
plt.plot(p + chunk0.shape[1] / 2)

# %%
p = online_register_rigid(lf2, batch_length=5000, mincorr=0.8, batch_size=64)
...
pof = po.copy()
good_times = (
    ((np.arange(len(pof)) > 50 * lfs) & (pof >= 20))
    | (np.arange(len(pof)) <= 50 *lfs)
)
good_times = (
    good_times
    & (
        ((np.arange(len(pof)) > 320 * lfs) & (pof > 108) & (pof < 113))
        | (np.arange(len(pof)) <= 320 * lfs)
    )
)
...
# %%
medcp, meds_est = median_changepoint(pof2[280 * lfs:])
...
# %%
pof3 = np.concatenate((pof2[:280 * lfs], meds_est))
...
plt.plot(pof2)
plt.plot(pof3)
plt.axhline(108, color="k")
plt.axhline(113, color="k")

# %%
np.flatnonzero(np.isnan(pof)).shape
...
ax.plot(300 + geom.max() / 2 + (p_precomputed - np.median(po)) * 25, color="b", lw=1, label="downsampled to 1Hz")
