...
sub_h5 = subtract.subtraction_binary(
    ...
    save_cleaned_pca_projs_on_n_channels=3,
)
...
# %%
plt.hist(p, bins=64)
plt.axvline(np.median(p), color="k")
mad = np.median(np.abs(p - np.median(p)))
robust_sigma_estimate = mad / 0.6745
# "median absolute deviation" MAD
cutoff = np.median(p) + 4 * robust_sigma_estimate
...
spike_train, templates, order = newms.new_merge_split(
    st,
    geom.shape[0],
    dsout / "sippx" / "traces_cached_seg0.raw", # binary file
    sub_h5, # path to h5 from subtraction
    geom,
    dsout / "clust", # output folder
    n_workers=1,
